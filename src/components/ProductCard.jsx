export default function ProductCard({ product }) {
  ...
  if (!product || !product.name || product.price == null) {
    return (
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 text-center text-gray-500 min-h-[320px] flex items-center justify-center">
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-50">
        <img
          src={product.image || "/images/placeholder-safety.jpg"}
          alt={product.name}
          className="
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          {product.badge && (
            <span className="bg-yellow-500 text-black text-xs font-bold uppercase px-3 py-1 rounded-full shadow-sm">
              {product.badge}
            </span>
          )}
          {product.certified && (
          ...
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            {oldPriceFormatted && (
              <span className="text-sm text-gray-500 line-through">
                KES {oldPriceFormatted}
              </span>
            )}
          </div>
        </div>

        {/* WhatsApp button – balanced for mobile */}
        <a
          href={`https://wa.me/2547XXXXXXXX?text=Hello%20SafetyPro%2C%20I'm%20interested%20in%20${encodeURIComponent(product.name)}%20–%20KES%20${formattedPrice}`}
          target="_blank"
          rel="noopener noreferrer"
          className="
            flex items-center justify-center gap-2.5
            bg-green-600 hover:bg-green-700 active:bg-green-800
            text-white font-medium text-[15px] md:text-sm
            py-3.5 px-5 rounded-lg
            transition-all duration-200
            active:scale-[0.98]
            focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2
            w-full
            md:w-auto md:py-3 md:px-4.5
            shadow-sm hover:shadow
          "
        >
          <svg
            className="w-5 h-5 md:w-4.5 md:h-4.5 flex-shrink-0"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
